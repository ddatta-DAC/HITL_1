...
# Linear classifier
# Two components:
# 1 .Feature interactions # interaction_type :: Choices 'concat', 'mul'
...
class linearClassifier_bEF(
    ...
    def __init__(
        ...
        self.binary_f_scale = binary_f_scale
    ...
    # -------------
    def fit(self, X, y, log_interval=250):
    ...
    # -------------
    # X has shape [ N, nd*(nd-1)/2, emb_dm ]
    # y has shape [N]
    # -------------
    def forward(self, x):
        feature_dim = int(self.num_domains * (self.num_domains-1)/2)
        _x_ = torch.chunk(x, feature_dim, dim=1)
    ...
    def setup_binaryFeatures(
        ...
        # This scaling factor is important  - since it assigns importance.
        # -------------------------------
        self.binary_W = np.ones(self.total_entity_count) * self.binary_f_scale
