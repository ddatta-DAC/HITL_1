...
def setup_config(DIR):
    ...
    test_data_loc = './../../generated_data_v1/{}/test_data.csv'.format(DIR)
...
def get_trained_classifier(
    ...
    if CLASSIFIER_TYPE =='RF':
        clf_obj = RandomForestClassifier(n_estimators=100)
        
        
    if CLASSIFIER_TYPE == 'KNN':
        from sklearn.neighbors import KNeighborsClassifier
        clf_obj =  KNeighborsClassifier(n_neighbors =10, p=2)
...
def execute_with_input(
    ...
    for b in range(max_num_batches):
        ...
        if x_test.shape[0]<1:break
        target_class_idx = clf_obj.classes_.tolist().index(1)

        new_scores = clf_obj.predict_proba(x_test)[:,target_class_idx]
        working_df.loc[:,'delta'] = new_scores
...
#     test_data_df = pd.read_csv(test_data_loc, index_col=None)
#     tmp = test_data_df.sample(n= len(anom_pos_df)*2)
#     for i in tqdm(range(tmp.shape[0])):
#         obj = record_class(tmp.iloc[i].to_dict(), -1)
#         obj_list.append(obj)
